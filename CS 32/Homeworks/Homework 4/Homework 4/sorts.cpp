...
void insertion_sort(vector<Student>& s, bool comp(const Student&, const Student&))
{
    ...
    for (int unsorted = 1; unsorted < s.size(); unsorted++){
        Student nextItem = s.at(unsorted);
        int loc = unsorted;
        while (loc > 0  &&  compareStudent(nextItem, s.at(loc-1))){
            s.at(loc) = s.at(loc-1);
            loc--;
        }
        s.at(loc) = nextItem;
    ...
    vector<Student> auxStudents(students.begin(), students.end());
      ...
      //        order.
        for (int i=0; i<students.size(); i++){
            students.at(i) = *(studentPtrs.at(i));
        }
